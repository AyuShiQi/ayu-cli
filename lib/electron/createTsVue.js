...
module.exports = function createElectron (propath, conpath) {
  ...
  // 初始化目录
  // 创建index.html
  createFileAndWrite(path.resolve(propath, './index.html'), path.resolve(conpath, './index.txt'))
  createSrc()
  createPlugins()
  ...
  function createSrc () {
    ...
    // 创建HelloWord.vue
    createFileAndWrite(path.resolve(curPath, './renderer/components/HelloWorld.vue'), path.resolve(conpath, './HelloWorld.txt'))
  }
